...
const BurgerLink = ({
  ...
  return (
    <div
      className="burgerlink act"
      style={openBurgerLink ? { display: "block" } : { display: "none" }}
    >
      <div className="burger_link_head">
        <img
          src={informationBurgerLink.ArrowRight}
          alt="arrow"
      ...
      <ul className="burger_link_list">
        {informationBurgerLink.firstList.map((item) => (
          <li>
            <NavLink
              className="burger_link"
              onClick={(e) => setTargetUrl(e)}
              to={`/catalog/${informationBurgerLink.id}/${item}`}
            >
              {item}
            </NavLink>
          </li>
